...
# Initialize MediaPipe Hands with optimized settings for speed and accuracy
mp_hands = mp.solutions.hands
hands = mp_hands.Hands(
    model_complexity=0,  # Lite model for speed
    max_num_hands=2,
    min_detection_confidence=0.5,  # Lower for faster detection
    min_tracking_confidence=0.7  # Higher for stable tracking, less jitter
)
...
thickness = 5
points = deque(maxlen=200)  # Store points for polylines
smooth_points = deque(maxlen=15)  # Balanced for smoothness without lag
prev_point = None
is_paused = False
last_gesture_time = 0
...
while True:
    ...
    rgb.flags.writeable = False  # Pass by reference for speed
